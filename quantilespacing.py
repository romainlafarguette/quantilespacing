...
June 2018
Time-stamp: "2021-11-07 11:41:01 RLafarguette"
"""
...
from statistics import NormalDist                        # Faster than scipy
norm = NormalDist(mu=0, sigma=1)                         # N(0,1) fast dist
...
def norm_ppf(tau):
    return(norm.inv_cdf(tau))


###############################################################################
#%% Single quantile interpolation
###############################################################################
def qs_ppf(alpha, qlist, condqlist, base=norm_ppf):
    """ 
...
###############################################################################
#@jit(fastmath=True) # I have problems with func signatures...
def qs_cdf(value, qlist, condqlist, base=norm_ppf, tol=1e-8, maxiter=500):
...
###############################################################################
#@jit(nopython=True, fastmath=True) # I have problems with func signatures...
def qs_ppf_mv(alpha, qlist, condqlist, base=norm_ppf, tol=1e-8, maxiter=500):
    """ 
...
###############################################################################
#@jit(nopython=True, fastmath=True) # I have problems with func signatures...
def qs_cdf_mv(value, qlist, condqlist, base=norm_ppf, tol=1e-8, maxiter=500):
    """ 
    Quantile Spacing CDF on a matrix of conditional quantiles
...
#%% Sampling
###############################################################################
def qs_sampling(qlist, condqlist, len_sample=1000, base=norm_ppf):
    """ 
